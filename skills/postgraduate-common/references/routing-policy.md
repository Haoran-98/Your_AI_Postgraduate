# Routing Policy

File notes by their role:

- Causal method, theorem, assumption, identification, DAG/SCM, benchmark -> `wiki/causal-core/`
- Domain variable or construct -> `wiki/variables/`
- Mechanism or causal pathway -> `wiki/mechanisms/`
- Intervention, policy, treatment, product action -> `wiki/interventions/`
- Dataset, measurement, benchmark -> `wiki/datasets/`
- Paper summary -> `wiki/papers/`
- Research idea -> `wiki/ideas/`
- Method applied to domain -> `wiki/causal-bridges/`

Before writing, read `wiki/index.md` and `wiki/hot.md` if they exist. After meaningful changes, update `index.md`, `hot.md`, and `log.md`.
